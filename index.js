...
CacheBuster.prototype.resources = function resources(options) {

  var basePath = options && options.base ? options.base : false;
  ...
  return through2.obj(function transform(file, encoding, callback) {
    ...
    if (basePath) {
      original = convertBasePath(original, basePath)
    }

    file.path = bustedPath;

    var slashed = slash(file.relative);
    if (basePath) {
      slashed = convertBasePath(slashed, basePath)
    }
    cachebuster.mappings[original] = slashed;
    this.push(file);
    return callback();
  });
};
...
function convertBasePath(original, basePath) {
  var originPath = original.slice(original.indexOf('/') + 1);
  var baseOriginPath = basePath.slice(basePath.indexOf('/') + 1);
  original = originPath.slice(baseOriginPath);
  return original;
}
